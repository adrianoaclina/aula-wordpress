...
function tela_dinamica(){
    ...
    global $wpdb;
    if(isset($_GET['buscar']) && !empty($_GET['campo'])){

        $where = "%{$_GET['campo']}%";
        
        $contatos = $wpdb->get_results($wpdb->prepare( "SELECT * FROM {$wpdb->prefix}agenda where nome like %s or whatsapp like %s", $where, $where));
    }else{
